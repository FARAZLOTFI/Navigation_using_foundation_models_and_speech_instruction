...
if __name__ == '__main__':
    ...
    while True:
        ...
        vehicles_poses.append([*pose, car_state.speed])
        ...
        if png is not None:
            actions, ref_speed = lseg_planner.planning(png, depthImage,  'human', measurements=observations,
                                                   unreal_mode=True, threshold=0.8,throttle_command=throttle)
        else:
            actions = [0.0,0.0]
            input('PNG is none!')
        ...
        print('Required speed is: ', ref_speed)
        if debug_mode:
